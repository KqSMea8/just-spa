...
function _writeComponent(componentInfo, componentPath) {
    ...
    const generatorJsonPath = __dirname + `/${componentInfo.template}/generator.json`;
    ...
    const fileList = generatorJsonFileList.map((filePath) => {
        return __dirname + `/${componentInfo.template}/${filePath}`;
    });

    for(let filePath of fileList){
        let fileName = filePath.replace(__dirname + `/${componentInfo.template}/`, '');
